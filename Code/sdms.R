...
load(file="/Users/katie/Documents/manuscript3/Taxa/taxa_over_20_occs_catchments.RData")
taxa <- t
head(taxa)
taxa$taxa_x <- gsub(" ", "_", taxa$taxa_c, fixed=T)

taxa171 <- read.csv("/Users/katie/Documents/manuscript3/Taxa/taxa_list_93.csv")
head(taxa171)
names(taxa171)
spp_list<- taxa171[,-1]
spp_list
...
setwd("/Users/katie/Documents/manuscript3/BRTs/2nd_run/wclim_hydro")
uni_vars <- read.csv(file="all_preds_for_sdms.csv")
head(uni_vars)
vars <- uni_vars$var
...
vars
# 
# [1] Bio09_worldclim  Bio04_worldclim  Bio08_worldclim 
# [4] Bio015_worldclim mh21             dh1             
# [7] Bio02_worldclim  
...
foreach(s=1:length(spp_list), .packages=c("sdm", "raster", "rgdal", "maptools")) %dopar% {
  ...
  ## write csv
  write.csv(vi_testx, paste(sx, "_wclim_only_var_imp_test.csv", sep=""))
  write.csv(vi_trainx, paste(sx, "_wclim_only_var_imp_train.csv", sep=""))
  
  # # ### create and save plots
  par(mar = rep(2, 4))
  jpeg(paste(sx,'_wclim_only_var_imp_auc_train.jpg', sep=""))
  plot(vi_train, 'auc')
  dev.off()
  #
  jpeg(paste(sx,'_wclim_only_var_imp_cor_train.jpg', sep=""))
  plot(vi_train, 'cor')
  dev.off()
  #
  jpeg(paste(sx,'_wclim_only_var_imp_auc_test.jpg', sep=""))
  plot(vi_test, 'auc')
  dev.off()
  #
  jpeg(paste(sx,'_wclim_only_var_imp_cor_test.jpg', sep=""))
  plot(vi_test, 'cor')
  dev.off()
  
  jpeg(paste(sx,'_wclim_only_roc.jpg', sep=""))
  roc(m2)
  dev.off()
  
  write.csv(sts, paste(sx, "_wclim_only_stats.csv",sep=""))
  
  e1 <- ensemble(m2,newdata=preds,filename= paste(sx, "_wclim_only_ensemble.grd",sep=""),
                 setting=list(method='weighted',stat='TSS',opt=2), overwrite=T) # paste(sx, "_uni_ensemble.tif",sep="")
